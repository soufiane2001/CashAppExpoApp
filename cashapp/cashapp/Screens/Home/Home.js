...
function Home({navigation,route }) {
    ...
    const openModal = (x,type) => {
     // setModalVisible(true);
      setType(type)
      setSelectenum(x)

      navigation.navigate('AddDepense',{type:type,num:selectenum})
    ...
    const onLayout = event => {
      ...
      const values = await AsyncStorage.getItem('userid');
        ...
        querySnapshot.forEach((doc) => {
         ...
         setBudget(parseInt(itemData.budgetinitial))  
         ...
         itemData.budget.map((x)=>{
          ...
          for(var i=0;i<months;i++){
            setBudget(budget=>budget+parseInt(x.revenu))  
          }
         ...
         itemData.depense.map((x)=>{
            x.depense.map((y)=>{
              setBudget(budget=>budget-parseInt(y.montant))  
            })
...
<Text style={{fontSize:getResponsiveFontSize(21),fontFamily:'PoppinsRegular',color:'#A1A09F'}}>Total Balance</Text>
...
<TouchableOpacity onPress={()=>{if(parseFloat(x.montant)==0){alert("il faut dans le budget")}if(parseFloat(x.montant)>parseFloat(m)){ openModal(index,x.type)}else{alert('vouz avez passe le limite')}} } style={{position:'absolute',zIndex:5565,left:'-2.5%'}}>
...
{m>0 &&
<LinearGradient  style={{opacity:0.2,borderRadius:getResponsiveFontSize(15),height:getResponsiveFontSize(103.5),width:`${ (((m/x.montant)*100+7)<100 ? ((m/x.montant)*100+7) : 107)}%`,backgroundColor:'white',position:'absolute',top:"0%",left:"0%",paddingHorizontal:"2%",flex:1,paddingVertical:"3%"}}
